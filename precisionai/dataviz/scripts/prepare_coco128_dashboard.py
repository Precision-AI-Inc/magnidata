...
"""Download COCO128-seg and build an dataviz dashboard dataset.

This script is intentionally standard-library-heavy so a user can run it from a
fresh checkout after installing the normal dataviz tool dependencies:

    python3 -m precisionai.dataviz.scripts.prepare_coco128_dashboard --device cpu
...
precisionai/dataviz/confi.yaml.
"""
...
def run_pipeline(
    ...
    # uninstalled checkout (see module docstring), so precisionai.dataviz.tools is not
    # importable until sys.path is patched with the repo root, immediately above.
    sys.path.insert(0, str(root))
    from precisionai.dataviz.tools import embeddings, features
    ...
    if args.with_nima:
        try:
            ...
            raise RuntimeError(
                ...
                "Install precisionai/dataviz/requirements.txt first."
            ) from exc
    ...
    for module_name in ("torch", "timm"):
        _ensure_importable(
            module_name,
            "Install precisionai/dataviz/requirements.txt or rerun with --no-embeddings.",
        )
...
def register_dataset(root: Path, stem: str, name: str, description: str) -> None:
    ...
    """
    config = root / "precisionai" / "dataviz" / "confi.yaml"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    """
    ap = argparse.ArgumentParser(description="Download COCO128 and build an dataviz dashboard dataset.")
    ...
    ap.add_argument("--description", default="Ultralytics COCO128 prepared for dataviz")
