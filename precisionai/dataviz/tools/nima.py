...
Heavy deps (torch + pyiqa) are imported lazily so the rest of the tool — and the
pixel/COCO/metadata/quality unit tests — work without them (use ``--skip-nima``,
which skips *all* learned IQA here).
...
class NimaScorer:
    def __init__(self, device: str = "cpu"):
        ...
        self.metrics = {col: pyiqa.create_metric(name, device=device).eval()
                        for name, col in VARIANTS if name not in _PER_CALL}
    ...
    def _to_tensor(self, image_path: str):
    ...
    def score(self, image_path: str) -> dict:
        ...
        with self._torch.no_grad():
            for name, col in VARIANTS:
                metric = (self._pyiqa.create_metric(name, device=self.device).eval()
                          if name in _PER_CALL else self.metrics[col])
