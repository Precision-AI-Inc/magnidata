...
Run:  python -m pytest precisionai/agriviz/api/test_app.py -q
"""
import importlib
import io
import json
...
def test_build_rejects_unknown_source_type(app_client):
    client, _ = app_client
    r = client.post("/api/datasets/build", data={"source_type": "bogus"},
...
def test_build_upload_returns_job_id(app_client, monkeypatch):
    ...
    monkeypatch.setattr(app_module.dataset_builder, "start_upload_build",
                         lambda images, annotations, embeddings_file, name, description: "job-123")

    r = client.post("/api/datasets/build", data={
        "source_type": "upload", "name": "Test Set",
...
def test_build_upload_passes_uploaded_files_with_relative_paths(app_client, monkeypatch):
    ...
    client.post("/api/datasets/build", data={
        "source_type": "upload", "name": "Test Set",
        "images": [
...
def test_build_demo_passes_demo_key(app_client, monkeypatch):
    ...
    r = client.post("/api/datasets/build", data={
        "source_type": "demo", "demo_key": "coco128",
    }, content_type="multipart/form-data")
...
def test_build_returns_400_on_validation_error(app_client, monkeypatch):
    ...
    r = client.post("/api/datasets/build", data={
        "source_type": "upload", "name": "", "images": (io.BytesIO(b"x"), "a.png"),
    }, content_type="multipart/form-data")
    assert r.status_code == 400
...
def test_build_returns_409_when_a_build_is_in_progress(app_client, monkeypatch):
    ...
    r = client.post("/api/datasets/build", data={"source_type": "demo", "demo_key": "coco128"},
                     content_type="multipart/form-data")
...
def test_get_build_job_returns_job_status(app_client, monkeypatch):
    client, app_module = app_client
    monkeypatch.setattr(app_module.dataset_builder, "get_job",
                         lambda job_id: {"status": "done", "percent": 100, "message": "Done",
                                         "dataset": {"id": 1}, "error": None})
...
def test_remove_dataset_cleans_up_built_dataset_files(app_client):
    ...
    with open(os.path.join(images_dir, "a.png"), "w") as f:
        f.write("x")
    rec = db.create_dataset_record("My Set", "", "data_user/myset.csv", None,
                                    "data_user/myset.json", 1)
...
def test_remove_dataset_child_only_removes_its_own_csv(app_client):
    ...
    rec = db.create_dataset_record("Child", "", "data_user/child.csv", "data/parent.csv",
                                    "data/parent.csv", 1)
