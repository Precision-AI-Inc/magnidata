...
| COCO labels | `<DATASET>/labels/<STEM>.json` (polygon segmentation) | instances, entanglement/overlap, crop-mimic colour similarity |
...
```bash
pip install -r precisionai/agriviz/requirements.txt   # torch CPU build: see file note
...
- **Coverage** (RGB+COCO): `green_annotation_ratio`, `annotated_px_count`, `green_mass`, `bg_coverage`
- **Instances / entanglement** (COCO): `instance_count`, `real_instance_count`, `mean_instance_area_ratio`, `instance_area_std`, `overlap_ratio`
- **Class / crop-mimic** (COCO+colour): `class_count`, `smallest_class_ratio`, `class_entropy`, `mean_pairwise_color_dist`, `interclass_color_sim`
...
- **Agronomic passthrough** (not computed): `weed_density`
...
```bash
python -m pytest precisionai/agriviz/tools/test_features.py -q
```
