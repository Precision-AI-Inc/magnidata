...
# dataset_builder must be imported before precisionai.agriviz.tools below — importing it
# inserts the precisionai package root onto sys.path (see its own _find_package_root),
# which that import relies on.
import dataset_builder
import db
import local_files
from precisionai.agriviz.tools import coco_labels as coco_labels_mod
...
def _mask_as_rgba(mask_data: bytes, max_size: int = 0) -> Image.Image:
    """Load an RGB(A) mask PNG and return an RGBA image where black/near-black
    background pixels are fully transparent and coloured foreground pixels
    keep their original colour at full (255) alpha.
    ...
    if max_size and max(result.size) > max_size:
        result.thumbnail((max_size, max_size), Image.NEAREST)
...
def get_image():
    ...
    try:
        ...
        buf  = io.BytesIO(_to_jpeg(img)); buf.seek(0)
...
def get_mask():
    ...
    try:
        ...
        buf     = io.BytesIO(_to_png(colored)); buf.seek(0)
...
def get_overlay():
    ...
    try:
        ...
        buf    = io.BytesIO(_to_jpeg(result)); buf.seek(0)
        return send_file(buf, mimetype="image/jpeg")
    except Exception as exc:
        return jsonify({"error": str(exc)}), 500
...
def _rasterize_annotation_mask(image_path: str, target_wh: tuple[int, int]) -> Image.Image | None:
    """Colored RGBA mask rasterized from the image's COCO label file — transparent
    where unlabeled/background, a distinct color per category elsewhere. None if the
    image has no label file.
    """
...
def _sibling_raster_mask_bytes(image_path: str) -> bytes | None:
    """Bytes of a pre-colored raster mask sibling to the image — <DATASET>/masks/<STEM>.ext
    next to <DATASET>/images/<STEM>.ext (the convention datasets like AgriStress-500 ship:
    one PNG per image, already colored per class, black = background — exactly
    _mask_as_rgba's format). Tries local paths and, for a remote image_path, the same
    allowlisted hosts /api/image already supports. None if no candidate is readable.
    """
    ...
    for ext in (".png", ".jpg", ".jpeg"):
        ...
        resolved = candidate if local_files.is_remote_url(image_path) else \
            os.path.normpath(os.path.join(DATA_ROOT, candidate))
        try:
...
def _load_annotation_mask(image_path: str, target_wh: tuple[int, int]) -> Image.Image | None:
    """The image's mask, from whichever source it has: real COCO polygon labels (exact
    per-instance shapes) take priority, falling back to a pre-colored raster mask file
    when there are no COCO labels. None if the image has neither.
    """
    ...
    if colored.size != target_wh:
        colored = colored.resize(target_wh, Image.NEAREST)
    return colored


@app.get("/api/annotation/mask")
def get_annotation_mask():
    """The colorized segmentation mask alone, as a transparent PNG."""
    path = request.args.get("path", "").strip()
    max_size = int(request.args.get("max_size", 1600))
    if not path:
        return jsonify({"error": "path required"}), 400
    ...
    try:
        ...
        buf = io.BytesIO(_to_png(mask)); buf.seek(0)
        return send_file(buf, mimetype="image/png")
    except Exception as exc:
        return jsonify({"error": str(exc)}), 500
...
def get_annotation_overlay():
    """The image with its segmentation mask composited on top at a caller-chosen
    opacity (0..1, default 0.5) — backs the preview modal's Overlay transparency
    slider.
    """
    path = request.args.get("path", "").strip()
    max_size = int(request.args.get("max_size", 1600))
...
def commit_partitions():
    """Commit a clustering: body {source, entries:[{image_name, cluster_id}]}. Upserts each row's
    cluster_id (overwriting a previous commit).
    """
    body = request.get_json(silent=True) or {}
...
def _resolve_emb_source(source: str) -> str | None:
    """The dataset path whose <stem>.json holds the embeddings for `source`.
...
def _detect_path_col(cols: list[str]) -> str | None:
    for c in cols:
        lc = c.lower()
...
def create_dataset():
    """Carve a child dataset from a parent's selection. Body: {name, description, parent_source,
    image_names:[...]}. Writes a child CSV (subset of the parent rows by image filename) and
    registers it; embeddings are inherited from the parent (resolved at compute time, not copied).
    """
    body = request.get_json(silent=True) or {}
    ...
    while os.path.isfile(os.path.normpath(os.path.join(DATA_ROOT, rel))) or db.get_created_dataset_by_source(rel):
        rel = f"{USER_DATA_REL}/{base_slug}_{n}.csv"; n += 1
...
def remove_dataset(ds_id: int):
    """Delete a user-created dataset. Confi datasets are not deletable. A row-subset
    child dataset (parent_source set) only removes its CSV — it inherits the parent's
    embeddings and was never staged from its own images. A dataset built from images
    (parent_source is None) also removes its embeddings JSON, staged images folder, and
    provenance sidecars.
    """
...
def list_demos():
    """Static registry of one-click demo datasets. Whether a given demo has already
    been built is NOT resolved here — the frontend checks GET /api/datasets for an
    existing dataset whose name matches a registry entry's name.
    """
...
def build_dataset():
    """Start a background dataset build. This pipeline never computes embeddings —
    bring your own. multipart form fields:
    ...
    try:
        if source_type == "upload":
            ...
            images = [
                dataset_builder.staging_mod.UploadedImage(relative_path=fs.filename or "image",
                                                           data=fs.read())
                for fs in request.files.getlist("images")
            ]
            annotations = [
                dataset_builder.staging_mod.UploadedImage(relative_path=fs.filename or "annotation",
                                                           data=fs.read())
            ...
            job_id = dataset_builder.start_upload_build(images, annotations, embeddings_file,
                                                         name, description)
...
def get_dataset_embeddings():
    ...
    Base embeddings = <stem>.json. An optional `variant` (model name) selects a comparison file
    <stem>_<variant>.json. 404 if it does not exist.
    """
    source = request.args.get("source", "").strip()
...
def _base_matrix(source: str, variant: str | None):
...
def embedding_projection():
    ...
    if method == "pca":
        P = PCA(n_components=3, random_state=0).fit_transform(M)
    else:
        pre = PCA(n_components=min(_PCA_PRE_DIMS, d), random_state=0).fit_transform(M) if d > _PCA_PRE_DIMS else M
        if method == "tsne":
            perp = max(5, min(30, (n - 1) // 3))
            P = TSNE(n_components=3, init="pca", perplexity=perp, random_state=0).fit_transform(pre)
        elif method == "lle":
            P = LocallyLinearEmbedding(n_components=3, n_neighbors=min(12, n - 1), random_state=0).fit_transform(pre)
        else:
            return jsonify({"error": "unknown method"}), 400
    positions = np.asarray(P, dtype=float).tolist()
...
def embedding_clusters():
    ...
    Mn = normalize(M)                                   # L2-normalize → KMeans on the unit sphere = cosine
    labels = KMeans(n_clusters=min(k, n), n_init=4, random_state=0).fit_predict(Mn)
...
def embedding_compare():
    """Place a comparison model's items in the ORIGINAL layout. For each comparison image we take its
    nearest neighbours IN THE COMPARISON SPACE and map them to the original rows by IMAGE NAME, so the
    frontend can interpolate their on-screen positions. Returns {nbr, w, origIdx} (all by row index).
    """
    try:
        source = _safe_source(request.args.get("source", "").strip())
    except ValueError as e:
    ...
    if not names or dim == 0:
        result = {"nbr": [], "w": [], "origIdx": []}
        _COMPUTE_CACHE[cache_key] = result
        return jsonify(result)

    C = np.array([comp[nm] for nm in names], dtype=np.float32)
    Cr = PCA(n_components=min(_PCA_PRE_DIMS, C.shape[1]), random_state=0).fit_transform(C) if C.shape[1] > _PCA_PRE_DIMS else C
    kk = min(7, len(names))                              # self + 6 neighbours
    nn = NearestNeighbors(n_neighbors=kk).fit(Cr)
    dist, idx = nn.kneighbors(Cr)
    ...
    for i in range(len(names)):
        ...
        if not neigh_rows:                               # fall back to the item's own row if known
            if row_of[i] >= 0:
        ...
        origIdx.append(row_of[i])
    result = {"nbr": nbr, "w": w, "origIdx": origIdx}
    _COMPUTE_CACHE[cache_key] = result
    return jsonify(result)
