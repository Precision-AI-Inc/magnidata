...
"""Download COCO128-seg and build an agriviz dashboard dataset.

This script is intentionally standard-library-heavy so a user can run it from a
fresh checkout after installing the normal agriviz tool dependencies:

    python3 -m precisionai.agriviz.scripts.prepare_coco128_dashboard --device cpu
...
precisionai/agriviz/confi.yaml.
"""
...
def read_yolo_seg_labels(path: Path) -> list[YoloInstance]:
    """Parse YOLO-segment label lines: ``class_id x1 y1 x2 y2 ... xn yn`` (normalized
    0..1 polygon vertices, n >= 3 per instance — Ultralytics' coco128-seg format).
    """
    if not path.is_file():
        return []
...
def polygon_area(points: Iterable[tuple[float, float]]) -> float:
    """Shoelace formula. Works in any consistent unit (normalized or pixel) — used
    both for real (pixel) area and for comparing instances by relative size.
    """
    pts = list(points)
    n = len(pts)
    s = 0.0
...
def write_coco_json(path: Path, image_name: str, width: int, height: int,
                     instances: list[YoloInstance]) -> None:
...
def pick_cluster(mode: str, image_path: Path, images_dir: Path,
                  instances: list[YoloInstance]) -> tuple[str, str]:
    if mode == "single":
        return "all", "all"
    if mode == "folder":
...
def stage_coco128(args: argparse.Namespace, root: Path, coco_root: Path,
                   stage_root: Path | None = None) -> tuple[Path, dict]:
    ...
    for src in images:
        ...
        checksum_lines.append(f"{sha256_file(src)}  {src.relative_to(root).as_posix()}")

    manifest.parent.mkdir(parents=True, exist_ok=True)
    with manifest.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "image_path", "cluster", "cluster_l2", "primary_class", "object_count", "source_path",
        ])
        writer.writeheader()
    ...
    with (stage_meta / "images_metadata.csv").open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=[
            "id", "original_name", "uuid", "camera_view", "angle", "gsd", "weed_density",
        ])
        writer.writeheader()
...
def run_pipeline(args: argparse.Namespace, root: Path, manifest: Path,
                  data_dir: Path | None = None) -> tuple[Path, Path | None]:
    sys.path.insert(0, str(root))
    from precisionai.agriviz.tools import embeddings, features
    ...
    if args.with_nima:
        try:
            ...
            raise RuntimeError(
                ...
                "Install precisionai/agriviz/requirements.txt first."
            ) from exc
    ...
    for module_name in ("torch", "timm"):
        try:
            ...
            raise RuntimeError(
                f"{module_name} is required for embeddings. Install precisionai/agriviz/requirements.txt "
                "or rerun with --no-embeddings."
            ) from exc
...
def register_dataset(root: Path, stem: str, name: str, description: str) -> None:
    config = root / "precisionai" / "agriviz" / "confi.yaml"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Download COCO128 and build an agriviz dashboard dataset.")
    ...
    ap.add_argument("--description", default="Ultralytics COCO128 prepared for agriviz")
    ...
    ap.add_argument("--stage-mode", choices=["copy", "hardlink", "symlink"], default="copy",
                    help="how to stage images under image_sets/<stem>/images")
    ap.add_argument("--cluster-by", choices=["dominant-class", "first-class", "folder", "single"],
    ...
    ap.add_argument("--with-nima", action="store_true",
    ...
    ap.add_argument("--validate-api", action="store_true",
                    help="validate the running dashboard API on localhost:5175")
