...
and registers the dataset in `precisionai/magnidata/confi.yaml`.
...
```bash
python -m precisionai.magnidata.scripts.prepare_coco128_dashboard \
...
```bash
python3.12 -m venv .venv-magnidata
source .venv-magnidata/bin/activate
...
python -m precisionai.magnidata.tools.features \
  --input "$INPUT_CSV" \
  --output "$FEATURE_CSV" \
...
Append the generated CSV to `precisionai/magnidata/confi.yaml`:
...
path = Path("precisionai/magnidata/confi.yaml")
text = path.read_text()
