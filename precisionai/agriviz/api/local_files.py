...
ALLOWED_REMOTE_HOSTS = {
    h.strip() for h in os.environ.get(
        "ALLOWED_REMOTE_IMAGE_HOSTS", "d379glmvb7evte.cloudfront.net",
    ).split(",") if h.strip()
}
...
def _thumb_candidates(path: str, thumb_root: str):
    """Thumbnail path candidates for a datalake image. Thumbnails mirror the datalake
    layout but are stored as .jpg (sources are .png), so try .jpg/.jpeg first, then the
    original extension.
    """
